...
**Note:** `SECEV4LIA_API_KEY` is optional. Without it, SecEv4LIA runs in local mode and stores all results in `~/.local/share/secev4lia/secev4lia.db`. If you need remote mode, pass `api_key` directly to `SecEv4LIA()`.
