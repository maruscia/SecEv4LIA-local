...
agent = SecEv4LIA(
    ...
    endpoint="http://localhost:8000",
    agent_type=AgentTypeEnum.GOOGLE_ADK,
    api_key=os.getenv("SECEV4LIA_API_KEY"),          # optional
)
```
...
# Optional: enable remote mode with an API key
export SECEV4LIA_API_KEY="your-api-key"        # omit for local mode
export OLLAMA_BASE_URL="http://localhost:11434"
```

**Note:** `SECEV4LIA_API_KEY` is optional. Without it, SecEv4LIA runs in local mode and stores all results in `~/.local/share/secev4lia/secev4lia.db`. Run `secev init` to configure defaults or pass `api_key` directly to `SecEv4LIA()`.
