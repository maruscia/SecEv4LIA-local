...
from secev4lia.server.api.apilogs import apilogs_list, apilogs_retrieve
from secev4lia.server.client import AuthenticatedClient
from secev4lia.errors import UnexpectedStatus
from secev4lia.server.api.models import APITokenLog
from secev4lia.server.api.models import PaginatedAPITokenLogList
...
def test_sync_success(authenticated_client: AuthenticatedClient):
    ...
    with patch(
        "secev4lia.server.api.apilogs.apilogs_list.sync_detailed",
...
async def test_asyncio_success(authenticated_client: AuthenticatedClient):
    ...
    with patch(
        "secev4lia.server.api.apilogs.apilogs_list.asyncio_detailed",
...
def test_retrieve_sync_success(authenticated_client: AuthenticatedClient):
    ...
    with patch(
        "secev4lia.server.api.apilogs.apilogs_retrieve.sync_detailed",
        return_value=mock_detailed_response,
    ) as mock_sync_detailed:
...
async def test_retrieve_asyncio_success(authenticated_client: AuthenticatedClient):
    ...
    with patch(
        "secev4lia.server.api.apilogs.apilogs_retrieve.asyncio_detailed",
        new=mock_asyncio_detailed,
    ) as _:
